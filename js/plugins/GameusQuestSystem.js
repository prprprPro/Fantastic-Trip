 ...
 *
 * Quest Open
 *   菜单中的任务栏开启,需要配合YEP_MainMenuManager插件开启.
 *
 * ----------------------------------------------------
 * 以下代码是脚本命令(一般在分支条件的脚本里添加):
 * ----------------------------------------------------
 ...
 *   翻译说和上面不一样...  
 *   判断多项任务处于什么状态(进行,失败,完成)
 *
 * $gameQuests.get(quest_id).currentStep === step_number
 *  判断任务步骤(从0开始)比如任务1的第2个步骤:($gameQuests.get(1).currentStep === 1).
 *
 * $gameQuests.get(quest_id).status === "status"
 *   获取任务的状态: "status" can be "progress(进行)", "completed(完成)", or "failed(失败)"
 *  
 * $gameQuests.totalQuests(filter)
 *   获取你获得全部任务数(根据filter变换获得全部,进行,成功,失败). Filter can be "all", "progress", "completed", or "failed". This applies to all quests.
 *
